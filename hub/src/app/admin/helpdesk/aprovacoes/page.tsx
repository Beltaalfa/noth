...
export default function AprovacoesPage() {
  ...
  useEffect(() => {
    if (clientId) {
      setLoading(true);
      Promise.all([fetchConfigs(), fetchDestinatarios()]).finally(() => setLoading(false));
    }
  }, [clientId, fetchConfigs, fetchDestinatarios]);

  const openCreate = () => {
    setEditing(null);
    setForm({ groupId: "", sectorId: "", exigeAprovacao: true, tipoAprovacao: "by_level", approvers: [] });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasGroup === hasSector) { toast.error("Informe grupo OU setor"); return; }
    setSaving(true);
    try {
      const payload = {
        clientId,
        ...(form.groupId ? { groupId: form.groupId } : { sectorId: form.sectorId }),
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-700/50">
                <th className="px-3 py-2 text-left font-medium text-zinc-400">Grupo/Setor</th>
            ...
            <tbody>
              {configs.map((c) => (
                <tr key={c.id} className="border-b border-zinc-700/30">
                  ...
                  <td className="px-3 py-2.5 text-zinc-400">{c.approvers.map((a) => a.user.name).join(", ") || "-"}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Editar configuração" : "Nova configuração"} maxWidth="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <p className="text-sm text-zinc-400 bg-zinc-800/50 rounded-lg px-3 py-2">
            Define quem deve aprovar tickets <strong>antes</strong> da execução. Escolha um grupo <strong>ou</strong> um setor, marque &quot;Exige aprovação&quot; e liste os aprovadores.
          </p>
          <div>
            <label className="block text-sm font-medium text-zinc-400 mb-1">Grupo ou Setor</label>
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-xs text-zinc-500">Grupo</label>
                <select value={form.groupId} onChange={(e) => setForm((f) => ({ ...f, groupId: e.target.value, sectorId: "" }))} className="w-full rounded-lg border border-zinc-600/80 bg-zinc-800/50 px-3 py-2 text-sm text-zinc-100" disabled={!!editing}>
                  <option value="">Nenhum</option>
                  {destinatarios?.groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
                </select>
              </div>
              <div className="flex-1">
                <label className="text-xs text-zinc-500">Setor</label>
                <select value={form.sectorId} onChange={(e) => setForm((f) => ({ ...f, sectorId: e.target.value, groupId: "" }))} className="w-full rounded-lg border border-zinc-600/80 bg-zinc-800/50 px-3 py-2 text-sm text-zinc-100" disabled={!!editing}>
                  <option value="">Nenhum</option>
